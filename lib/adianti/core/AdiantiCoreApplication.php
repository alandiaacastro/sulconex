...
class AdiantiCoreApplication
{
    ...
    public static function run($debug = FALSE)
    {
        ...
        self::filterInput();
        $class = self::resolveLegacyClass($class);
        ...
        $rc = !empty($class) && class_exists($class) ? new ReflectionClass($class) : null;
        ...
        if ($rc && in_array(strtolower($class), array_map('strtolower', AdiantiClassMap::getInternalClasses()) ))
        {
            ob_start();
    ...
    /**
     * Resolve legacy controllers that were intentionally removed
     */
    private static function resolveLegacyClass($class)
    {
        $legacy_routes = [
            'CctCertificateConfigForm' => 'Dashboard',
            'CctTransmissaoForm'       => 'Dashboard',
            'CctTransmissaoList'       => 'Dashboard',
            'CctManualForm'            => 'Dashboard',
            'CctUploadXmlForm'         => 'Dashboard',
        ];
        
        return $legacy_routes[$class] ?? $class;
    }
