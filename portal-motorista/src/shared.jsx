import { ROUTES, buildQueryString, formatDate, getErrorMessage, getPortalConfig, greeting, maskPhone } from './lib/runtime.js';

(() => {
  const config = getPortalConfig();
  window.PORTAL_MOTORISTA_CONFIG = config;
  ...
  const ICON_PATHS = {
    ...
    lock: 'M7.5 10.5V8.25a4.5 4.5 0 1 1 9 0v2.25m-9 0h9m-9 0A1.5 1.5 0 0 0 6 12v6a1.5 1.5 0 0 0 1.5 1.5h9A1.5 1.5 0 0 0 18 18v-6a1.5 1.5 0 0 0-1.5-1.5',
  ...
  function summarizeInvalidResponse(text, status) {
    ...
    if (!normalized) {
      return status ? `Resposta invalida do servidor. (HTTP ${status})` : 'Resposta invalida do servidor.';
    }

    const excerpt = normalized.length > 180 ? `${normalized.slice(0, 177)}...` : normalized;
    return `Resposta invalida do servidor. ${excerpt}`;
  }
  ...
  async function apiRequest(path, options = {}) {
    ...
    if (!payload) {
      const error = new Error(response.ok ? summarizeInvalidResponse(text, response.status) : `Erro ${response.status}`);
  ...
  function RequestErrorState({ title, description, onRetry, retryLabel = 'Tentar novamente' }) {
    return (
      <EmptyState
        title={title}
  ...
  function useRemoteData(loadData) {
    const [state, setState] = React.useState({ loading: true, data: null, error: '' });
    const [reloadVersion, bumpReloadVersion] = React.useReducer((value) => value + 1, 0);
