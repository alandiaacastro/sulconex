<?php
return [
    'general' =>  [
        'timezone' => 'America/Sao_Paulo',
        'language' => 'pt',
        'application' => 'template',
        'title' => 'OMD TRUCK 8.4',
        'theme' => 'adminbs5',
        'seed' => 'odfu6asnodf8as',
        'rest_key' => '',
        'multiunit' => '0',
        'public_view' => '0',
        'public_entry' => '',
        'debug' => '0',
        'strict_request' => '0',
        'multi_lang' => '0',
        'require_terms' => '0',
        'concurrent_sessions' => '1',
        'lang_options' => [
          'pt' => 'Português',
          'en' => 'English',
          'es' => 'Español',
          'fr' => 'Français',
          'de' => 'Deutsch',
          'it' => 'Italiano',
        ],
        'multi_database' => '0',
        'validate_strong_pass' => '1',
        'notification_login' => '0',
        'welcome_message' => 'bom trabalho!',
        'request_log_service' => 'SystemRequestLogService',
        'request_log' => '0',
        'request_log_types' => 'cli,web,rest',
        /*'password_renewal_interval' => '',*/
    ],
    'recaptcha' => [
        'enabled' => '0',
        'key' => '...',
        'secret' => '...'
    ],
    'permission' =>  [
        'public_classes' => [
          'SystemRequestPasswordResetForm',
          'SystemPasswordResetForm',
          'SystemRegistrationForm',
          'SystemPasswordRenewalForm',
          'SystemConcurrentAccessView',
          'PortalMotoristaHome',
          'PortalMotoristaLogin',
          'PortalMotoristaSolicitarCadastro',
          'PortalMotoristaCargas',
          'PortalMotoristaAndamento',
          'PortalMotoristaContratos',
          'PortalMotoristaSolicitacoes',
          'PortalMotoristaDocumentos',
        ],
        'user_register' => '0',
        'reset_password' => '0',
        'default_groups' => '2',
        'default_screen' => '30',
        'default_units' => '1',
    ],
    'highlight' => [
        'comment' => '#808080',
        'default' => '#FFFFFF',
        'html' => '#C0C0C0',
        'keyword' => '#62d3ea',
        'string' => '#FFC472',
    ],
    'login' => [
        'logo' => 'app\images\logobranco.png',
        'background' => ''
    ],
    'template' => [
        'navbar' => [
            'has_program_search' => '0',
            'has_notifications' => '1',
            'has_messages' => '1',
            'has_docs' => '1',
            'has_contacts' => '1',
            'has_support_form' => '1',
            'has_wiki' => '1',
            'has_news' => '1',
            'has_menu_mode_switch' => '1',
            'has_main_mode_switch' => '1',
            'has_master_menu' => '1',
            'always_collapse' => '0',
            'allow_page_tabs' => '0',
            'only_top_menu' => '0'
        ],
        'dialogs' => [
            'use_swal' => '1'
        ],
        'theme' => [
            'menu_dark_color' => 'rgb(29 45 83)',
            'login_mode'  => 'light',
            'menu_mode'  => 'dark',
            'main_mode'  => 'light',
            'box_layout' => '0'
        ]
    ]
];
