...
class PortalMotoristaCargas extends TPage
{
    ...
    public function __construct()
    {
        ...
        $container->add($this->cardsContainer);
    ...
    public function onReload($param = null)
    {
        try {
            ...
            $motorista = Motorista::getBySystemUser(TSession::getValue('userid'));
    ...
    private function buildCard($carga, $motorista)
    {
        ...
        $aduanaOrig = !empty($carga->aduana_origem) ? htmlspecialchars($carga->aduana_origem) : null;
        $aduanaDest = !empty($carga->aduana_destino) ? htmlspecialchars(CargaDisponivel::getAduanaDestinoItems()[$carga->aduana_destino] ?? $carga->aduana_destino) : null;
        ...
        if ($aduanaDest) {
            $body->add("<div class='info-row'><i class='fas fa-map-marker-alt'></i><span class='label'>Aduana:</span> {$aduanaDest}</div>");
        }
