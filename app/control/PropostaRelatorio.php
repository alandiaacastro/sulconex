...
class PropostaRelatorio extends TPage
{
    ...
    public static function onImprimir($param): void
    {
        ...
        try {
            ...
            // Busca cidade de origem (Rota 1) e destino (Rota 2) da TabelaFrete
            $p->rota1_origem   = '';
            $rota1_destino = '';
            $p->rota1_fronteira = '';
            $p->rota2_destino  = '';
            if (!empty($p->frete_origem_id)) {
                try {
                    $f1 = new TabelaFrete($p->frete_origem_id);
                    $p->rota1_origem    = trim((string) ($f1->origem ?? ''));
                    $rota1_destino      = trim((string) ($f1->destino ?? ''));
                    $p->rota1_fronteira = self::firstFilled([$f1->fronteira ?? '', $rota1_destino], '');
                } catch (Exception $e) {}
            }
            if (!empty($p->frete_destino_id)) {
                try {
                    $f2 = new TabelaFrete($p->frete_destino_id);
                    $p->rota2_destino = trim((string) ($f2->destino ?? ''));
                } catch (Exception $e) {}
            }

            $p->rota1_origem = self::firstFilled([$p->rota1_origem, $p->Local_Coleta ?? ''], '');
            $p->rota1_fronteira = self::firstFilled([$p->rota1_fronteira, $p->Aduana_Fronteira ?? ''], '');
            $p->rota2_destino = self::firstFilled([$p->rota2_destino, $p->Local_Entrega ?? '', $rota1_destino], '');
    ...
    private static function renderLogistica(FPDF $pdf, Proposta $p, float &$y): void
    {
        ...
        self::renderLogisticaLinhaDupla(
            ...
            $y,
            'Rota 1 — Origem',
            (string) ($p->rota1_origem ?? ''),
            'Fronteira / Aduana',
            (string) ($p->rota1_fronteira ?? $p->Aduana_Fronteira ?? '')
        );
