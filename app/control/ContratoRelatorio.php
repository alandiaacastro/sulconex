...
class ContratoRelatorio extends TPage
{
    ...
    private static function gerarPDF($object)
    {
        ...
        
        {
            ...
            $pdf->Text(50, 14, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'COMPROVANTE DE PAGAMENTO DE FRETE'));
            $pdf->SetFont('Helvetica', 'B', 8);
            $pdf->Text(50, 18, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'INSTRUMENTO DE SUBCONTRATAÇÃO DE TRANSPORTE RODOVIÁRIO'));
            $pdf->SetTextColor(0, 0, 0);
            $pdf->SetLineWidth(0.1);

            $pdf->Rect(150, 13, 50, 6, 'D');
            $pdf->SetFont('Arial', 'B', 8);
            $pdf->Text(152, 12, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'N° PAGAMENTO'));
            $pdf->SetFont('Arial', 'B', 9);
            $pdf->Text(154, 17, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'CT ' .($object->conhecimento_numero ?? '---').'-'.  str_pad($object->id, 3, '0', STR_PAD_LEFT) . '/' . date('Y')));
...
$pdf->SetFont('Arial', '', 6);
$pdf->Text(11, 24, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','CONTRATANTE'));
$pdf->SetFont('Arial', '', 8);
$pdf->SetXY(11, 26); 
$pdf->MultiCell(68, 4, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', (string) ($permisso->dados_documentos ?? '')), 0, 'L');

// ðŸ”¹ Contratado
// Este quadro já define a altura de referência (26).
$pdf->Rect(80, 25, 120, 26, 'D');
$pdf->SetFont('Arial', '', 6);
$pdf->Text(82, 24, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','CONTRATADO'));
$pdf->Text(82, 28, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','MOTORISTA'));
$pdf->SetFont('Arial', 'B', 8);
$pdf->Text(82, 31, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', ($object->veiculo->motorista->nome ?? '') . ' CPF: ' . ($object->veiculo->motorista->cpf ?? '')));

$pdf->SetFont('Arial', '', 6);
$pdf->Text(82, 34, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','PROPRIETARIO'));
$pdf->SetFont('Arial', 'B', 8);
$pdf->Text(82, 37, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', ($object->veiculo->proprietario->razao_social ?? '')));
$pdf->Text(82, 40, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', ( 'CNPJ: ' . ($object->veiculo->proprietario->cnpj ?? ''))));

$pdf->SetFont('Arial', '', 6);
$pdf->Text(82, 44, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'VEICULO'));
$pdf->SetFont('Arial', 'B', 8);
$pdf->Text(82, 47, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', ($object->veiculo->placa_trator ?? '').' / '.($object->veiculo->antt_consulta_semi_reboque->placa ?? '')));

$pdf->SetFont('Arial', '', 6);
$pdf->Text(115, 44, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'CHASSIS TRATOR'));
$pdf->SetFont('Arial', 'B', 8);
$pdf->Text(115, 47, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', (string) ($object->veiculo->antt_consulta_trator->chassi_motor ?? '')));

$pdf->SetFont('Arial', '', 6);
$pdf->Text(151, 44, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'MARCA/MODELO/ANO'));
$pdf->SetFont('Arial', 'B', 8);
$marcaModeloAno = ($object->veiculo->antt_consulta_trator->marca ?? '') . ' / '  . ($object->veiculo->ano_fabricacao ?? '');
$pdf->Text(151, 47, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', $marcaModeloAno));
            // --- FIM DA ALTERAÃ‡ÃƒO ---

            $pdf->SetFont('Arial', 'B', 9);
            $pdf->SetXY(10, 54);
            $pdf->Cell(0, 0, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', '2 - CLÁUSULAS DO CONTRATO'));
            ...
            $pdf->SetFont('Arial', 'B', 9); $pdf->Text(10, $pdf->GetY(), @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'TABELA DE FRANQUIAS LIVRES (SEM ESTADIAS)'));
            $pdf->Rect(10, $pdf->GetY()+2, 190, 10, 'D');
            $pdf->SetFont('Arial', '', 6); $pdf->Text(11, $pdf->GetY()+4, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'MULTILOG')); $pdf->Text(42, $pdf->GetY()+4, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'ADUANA ARGENTINA')); $pdf->Text(73, $pdf->GetY()+4, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'ADUANA CHILE')); $pdf->Text(104, $pdf->GetY()+4, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','DESCARGA')); $pdf->Text(135, $pdf->GetY()+4, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','DIARIAS ARGENTINA')); $pdf->Text(166, $pdf->GetY()+4, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','DIARIAS CHILE'));
            $pdf->SetFont('Arial', 'B', 10); $pdf->Text(16, $pdf->GetY()+9, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', '48H')); $pdf->Text(48, $pdf->GetY()+9, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', '48H')); $pdf->Text(79, $pdf->GetY()+9, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','48H')); $pdf->Text(110, $pdf->GetY()+9, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','24H')); $pdf->Text(145, $pdf->GetY()+9, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','R$ 450,00')); $pdf->Text(176, $pdf->GetY()+9, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','R$ 600,00'));

            // Rota e Documentos
            $pdf->SetY($pdf->GetY() + 16);
            $pdf->SetFont('Arial', 'B', 9); $pdf->Text(10, $pdf->GetY(), @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', '3 -ROTA PERCURSO E DOCUMENTOS DO EMBARQUE'));
            $pdf->Rect(10, $pdf->GetY()+2, 30, 10, 'D'); $pdf->Rect(41, $pdf->GetY()+2, 50, 10, 'D'); $pdf->Rect(92, $pdf->GetY()+2, 108, 10, 'D');
            $pdf->SetFont('Arial', '', 6); $pdf->Text(16, $pdf->GetY()+4, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','CRT')); $pdf->Text(46, $pdf->GetY()+4, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','NOTA FISCAL OU MIC/DTA')); $pdf->Text(97, $pdf->GetY()+4, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','ROTA PERCURSO'));
            $pdf->SetFont('Arial', 'B', 9); $pdf->Text(14, $pdf->GetY()+9, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', (string) ($object->conhecimento_numero ?? ''))); $pdf->Text(46, $pdf->GetY()+9, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', (string) ($object->danfeoumic ?? '')));
            $pdf->SetFont('Arial', '', 9); $pdf->Text(97, $pdf->GetY()+9, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', ' De '.($object->origem1 ?? '').' Até ' .($object->destino1 ?? '')));

            // Recibo
            $pdf->SetY($pdf->GetY() + 16);
            $pdf->SetFont('Arial', 'B', 9); $pdf->Text(10, $pdf->GetY(), @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','RECIBO DE PAGAMENTO DE FRETE - CONTRATO '. str_pad($object->id, 3, '0', STR_PAD_LEFT) . '/' . date('Y')));
            $pdf->Rect(10, $pdf->GetY()+2, 60, 31, 'D'); $pdf->Rect(71, $pdf->GetY()+2, 129, 8, 'D'); $pdf->Rect(71, $pdf->GetY()+2, 129, 31, 'D');

            $pdf->SetFont('Arial', '', 9);
            $pdf->Text(11, $pdf->GetY()+6, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','(+) VALOR FRETE')); $imprimirValorAlinhado($pdf, 66, $pdf->GetY()+6, 'R$ ' . number_format((float)($object->frete1 ?? 0), 2, ',', '.'));
            $pdf->Text(11, $pdf->GetY()+10, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','(-) ADTO FRETE')); $imprimirValorAlinhado($pdf, 66, $pdf->GetY()+10, 'R$ ' . number_format((float)($object->adt1 ?? 0), 2, ',', '.'));
            $pdf->Text(11, $pdf->GetY()+14, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','(-) INSS')); $imprimirValorAlinhado($pdf, 66, $pdf->GetY()+14, 'R$ ' . number_format((float)($object->inss1 ?? 0), 2, ',', '.'));
            $pdf->Text(11, $pdf->GetY()+18, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','(-) IRRF')); $imprimirValorAlinhado($pdf, 66, $pdf->GetY()+18, 'R$ ' . number_format((float)($object->irrf1 ?? 0), 2, ',', '.'));
            $pdf->Text(11, $pdf->GetY()+22, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','(-) SEST/SENAT')); $imprimirValorAlinhado($pdf, 66, $pdf->GetY()+22, 'R$ ' . number_format((float)($object->sest1 ?? 0), 2, ',', '.'));
            $pdf->Text(11, $pdf->GetY()+26, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','(-) DESCONTOS')); $imprimirValorAlinhado($pdf, 66, $pdf->GetY()+26, 'R$ ' . number_format((float)($object->descontos1 ?? 0), 2, ',', '.'));
            $pdf->Text(11, $pdf->GetY()+30, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE','(=) SALDO')); $imprimirValorAlinhado($pdf, 66, $pdf->GetY()+30, 'R$ ' . number_format((float)($object->saldo1 ?? 0), 2, ',', '.'));

            $pdf->SetFont('Arial', '',6); $pdf->Text(73, $pdf->GetY()+4, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'Valor por extenso'));
            $pdf->SetFont('Arial', '',8); $pdf->Text(73, $pdf->GetY()+8, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', (string) ($object->extenso1 ?? '')));
            $pdf->SetFont('Arial', '',6); $pdf->Text(73, $pdf->GetY()+12, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'Forma de pagamento'));
            $pdf->SetFont('Arial', '',8); $pdf->SetXY(73, $pdf->GetY()+14); $pdf->MultiCell(120, 5, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', (string) ($object->pagamento ?? '')), 0, 'L'); 
        ...
        $pdf->SetX(10);
        $pdf->MultiCell(190, 4, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', $textocontrato), 0, 'L');
        ...
        $pdf->Text(14, $y_assinatura, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'Uruguaiana, RS ' . $dataFormatada));
        ...
        $pdf->Text(10, 290, @iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', 'ORIGINAL'));
        }
