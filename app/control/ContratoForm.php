...
class ContratoForm extends TPage
{
    ...
    public function __construct($param)
    {
        ...
        $origem1 = new THidden('origem1');
        $destino1 = new THidden('destino1');
        $frete_id = new TDBCombo('frete_id', 'sample', 'TabelaFrete', 'id', '{origem} → {destino}', 'id desc');
        ...
        $frete_tonelada = new TCombo('frete_tonelada');
        $frete_tonelada->addItems(['N' => 'Frete Fixo', 'S' => 'Por Tonelada']);
        $peso_tonelada  = new TNumeric('peso_tonelada', 3, ',', '.', true);
        $valor_por_ton  = new TNumeric('valor_por_ton', 2, ',', '.', true);
        ...
        $frete_id = new TDBCombo('frete_id', 'sample', 'TabelaFrete', 'id', TabelaFrete::COMBO_MASK, 'id desc');
        ...
        $frete_action = new TAction(['ContratoForm', 'onTabelaFreteSelect']);
        $frete_id->setChangeAction($frete_action);

        $frete_tonelada->setChangeAction($update_action);
        $peso_tonelada->setExitAction($update_action);
        $valor_por_ton->setExitAction($update_action);
        ...
        $this->form->addFields( [$origem1], [$destino1] );

        $this->form->appendPage('Valores e Pagamento');
        $this->form->addFields( [new TLabel('Tipo de Frete')], [$frete_tonelada], [new TLabel('Peso (ton)')], [$peso_tonelada] );
        $this->form->addFields( [new TLabel('Valor/Ton (Tabela)')], [$valor_por_ton] );
    ...
    {
        $frete_ton      = trim($param['frete_tonelada'] ?? 'N');
        $peso           = self::parseBrNumber($param['peso_tonelada'] ?? 0);
        $vpt            = self::parseBrNumber($param['valor_por_ton']  ?? 0);
        ...
        // Recalcula frete quando modo tonelada ativo
        if ($frete_ton === 'S' && $peso > 0 && $vpt > 0) {
            $frete = round($peso * $vpt, 2);
        }
        ...
        // Recalcula frete quando modo tonelada ativo
        if ($frete_ton === 'S' && $peso > 0 && $vpt > 0) {
            $obj->frete1 = number_format($frete, 2, ',', '.');
        }
    ...
    public static function onTabelaFreteSelect($param): void
    {
        $frete_id = $param['frete_id'] ?? null;
        if (is_array($frete_id)) {
            $frete_id = reset($frete_id);
        }
        if (empty($frete_id)) {
            return;
        }
        ...
        try {
            TTransaction::open('sample');
            $tfrete = new TabelaFrete((int) $frete_id);
            TTransaction::close();

            if (!empty($tfrete->id)) {
                $obj = new stdClass;
                $obj->valor_por_ton = number_format((float) $tfrete->valor_frete, 2, ',', '.');
                $obj->origem1  = (string) $tfrete->origem;
                $obj->destino1 = (string) $tfrete->destino;

                $frete_ton = trim($param['frete_tonelada'] ?? 'N');
                $peso      = self::parseBrNumber($param['peso_tonelada'] ?? 0);
                if ($frete_ton === 'S' && $peso > 0) {
                    $obj->frete1 = number_format((float) $tfrete->valor_frete * $peso, 2, ',', '.');
                } else {
                    $obj->frete1 = number_format((float) $tfrete->valor_frete, 2, ',', '.');
                }
                TForm::sendData('form_contrato', $obj, false, false);
            }
        } catch (Exception $e) {
    ...
    public function onSave($param)
    {
        try {
            ...
            if (!empty($object->frete_id)) {
                ...
                if (!empty($frete->id)) {
                    $object->valor_por_ton = (float) ($frete->valor_frete ?? 0);
                    $object->origem1       = (string) ($frete->origem ?? '');
                    $object->destino1      = (string) ($frete->destino ?? '');
                    if (!empty($object->frete_tonelada) && $object->frete_tonelada === 'S' && (float)$object->peso_tonelada > 0) {
                        $object->frete1 = round($object->valor_por_ton * (float) $object->peso_tonelada, 2);
                    } else {
                        $object->frete1 = (float) ($frete->valor_frete ?? 0);
                    }
                }
            }
    ...
    public function onEdit($param)
    {
        try {
            if (isset($param['key'])) {
                ...
                $this->form->setData($object);
                $freteId = $object->frete_id ?? null;
                if (empty($freteId) && !empty($object->origem1) && !empty($object->destino1)) {
                    $freteId = TabelaFrete::findLatestRouteIdByData(
                        TTransaction::get(),
                        (string) $object->origem1,
                        (string) $object->destino1,
                        null,
                        isset($object->frete1) ? (float) $object->frete1 : null
                    );
                }
                if (!empty($freteId)) {
                    $obj = new stdClass;
                    $obj->frete_id = (int) $freteId;
                    TForm::sendData('form_contrato', $obj, false, false);
                }
