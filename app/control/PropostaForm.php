...
class PropostaForm extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        try {
            ...
            // Fronteiras = todos os destinos únicos que também aparecem como origens (interseção)
            $result = $conn->query("
                SELECT DISTINCT destino AS fronteira FROM tabela_fretes
                WHERE destino IS NOT NULL AND destino != ''
                  AND destino IN (SELECT DISTINCT origem FROM tabela_fretes WHERE origem IS NOT NULL AND origem != '')
                ORDER BY fronteira
            ");
            $fronteiras = ['' => '-- Selecione --'];
            foreach ($result as $row) {
                $v = trim($row['fronteira']);
                if ($v !== '') $fronteiras[$v] = $v;
            }
        ...
        $frete_origem_id = new TDBCombo('frete_origem_id', self::$database, 'TabelaFrete', 'id', '{origem} → {destino}');
        $frete_origem_id = new TDBCombo('frete_origem_id', self::$database, 'TabelaFrete', 'id', TabelaFrete::COMBO_MASK);
        ...
        $frete_destino_id = new TDBCombo('frete_destino_id', self::$database, 'TabelaFrete', 'id', '{origem} → {destino}');
        $frete_destino_id = new TDBCombo('frete_destino_id', self::$database, 'TabelaFrete', 'id', TabelaFrete::COMBO_MASK);
        ...
        $button_rota->style = 'height: 32px; padding-top: 5px;';
        ...
        $row_adu->layout = ['col-sm-4', 'col-sm-8'];

        $row_rotas = $this->form->addFields(
            [new TLabel('Rota 1 — Frete Origem'),  $frete_origem_id],
            [new TLabel('Rota 2 — Frete Destino'), $frete_destino_id],
            [new TLabel('&nbsp;'),                  $button_rota]
        );
        $row_rotas->layout = ['col-sm-5', 'col-sm-5', 'col-sm-2'];
    ...
    public static function onChangeFreteOrigem($param): void
    {
        if (!empty($param['frete_origem_id'])) {
            try {
                ...
                $obj->Aduana_Fronteira = self::resolveAduanaFromFrete($frete);
    ...
    private static function syncRouteFields(Proposta $proposta): void
    {
        $rotaOrigemDestino = '';

        if (!empty($proposta->frete_origem_id)) {
            $rotaOrigem = new TabelaFrete((int) $proposta->frete_origem_id);
            if (!empty($rotaOrigem->id)) {
                $proposta->frete_origem = (float) ($rotaOrigem->valor_frete ?? 0);
                $rotaOrigemDestino = trim((string) ($rotaOrigem->destino ?? ''));

                $origem = trim((string) ($rotaOrigem->origem ?? ''));
                if ($origem !== '') {
                    $proposta->Local_Coleta = $origem;
                }

                $aduana = self::resolveAduanaFromFrete($rotaOrigem);
                if ($aduana !== '') {
                    $proposta->Aduana_Fronteira = $aduana;
                }
            }
        }

        if (!empty($proposta->frete_destino_id)) {
            $rotaDestino = new TabelaFrete((int) $proposta->frete_destino_id);
            if (!empty($rotaDestino->id)) {
                $proposta->frete_destino = (float) ($rotaDestino->valor_frete ?? 0);

                $destino = trim((string) ($rotaDestino->destino ?? ''));
                if ($destino !== '') {
                    $proposta->Local_Entrega = $destino;
                }
            }
        } elseif ($rotaOrigemDestino !== '' && trim((string) ($proposta->Local_Entrega ?? '')) === '') {
            $proposta->Local_Entrega = $rotaOrigemDestino;
        }
    }

    private static function resolveAduanaFromFrete(?TabelaFrete $frete): string
    {
        if (!$frete) {
            return '';
        }

        $fronteira = trim((string) ($frete->fronteira ?? ''));
        if ($fronteira !== '') {
            return $fronteira;
        }

        return trim((string) ($frete->destino ?? ''));
    }
    /**
     * Limpa o formulario
     */
    ...
    public function onMapa($param = null): void
    {
        ...
        try {
            TTransaction::open(self::$database);
            if (!empty($param['frete_origem_id'])) {
                $f1 = new TabelaFrete($param['frete_origem_id']);
                $origem        = trim((string) $f1->origem);
                $intermediario = self::resolveAduanaFromFrete($f1);
            }
            if (!empty($param['frete_destino_id'])) {
                $f2 = new TabelaFrete($param['frete_destino_id']);
                $destino = trim((string) $f2->destino);
            }
            TTransaction::close();
        } catch (Exception $e) {
            TTransaction::rollback();
        ...
        if ($origem && $destino) {
            ...
            new TMessage('warning', 'Selecione ao menos <strong>Rota 1</strong> e <strong>Rota 2</strong> para visualizar a rota no mapa.');
        }
    }
    ...
    public static function onCadastrarRota($param = null): void
    {
        TabelaFreteForm::openPanel(self::$formName);
    }

    public static function onReloadFretesCombos($param = null): void
    {
        try {
            TTransaction::open(self::$database);
            TDBCombo::reloadFromModel(self::$formName, 'frete_origem_id',  self::$database, 'TabelaFrete', 'id', '{origem} → {destino}', 'id desc');
            TDBCombo::reloadFromModel(self::$formName, 'frete_destino_id', self::$database, 'TabelaFrete', 'id', '{origem} → {destino}', 'id desc');
            TTransaction::close();
        } catch (Exception $e) {
            TTransaction::rollback();
        }
