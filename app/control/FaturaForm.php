...
class FaturaForm extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $this->form->appendPage('Valores e Observacoes');
        ...
        // Fix TNumeric em abas ocultas: reaplica valor via keyup ao trocar de aba
        TScript::create("
            (function() {
                function refreshNumericFields() {
                    var fields = ['valor1','valor2','valor3','valor_fatura','desconto_banco','valor_liquido'];
                    fields.forEach(function(name) {
                        ...
                        if (el && el.value) {
                            ...
                            if (typeof jQuery !== 'undefined') {
                                jQuery(el).trigger('keyup').trigger('change').trigger('blur');
                            }
                        }
                    });
                }
                setTimeout(function() {
                    var tabLinks = document.querySelectorAll('form[name=form_fatura] .nav-link');
                    tabLinks.forEach(function(tab) {
                        tab.addEventListener('shown.bs.tab', function() { setTimeout(refreshNumericFields, 100); });
                        tab.addEventListener('click', function() { setTimeout(refreshNumericFields, 150); });
                    });
