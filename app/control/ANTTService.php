...
/*
 * =================================================================
 * CÃ“DIGO DEFINITIVO PARA CONSULTA ANTT (COM URL CORRIGIDA)
 * Autor: Parceiro de Programação
 * Data: 21/06/2025
 * Lógica: URL corrigida e simulação de navegador em 2 etapas
 * =================================================================
 */
class ANTTService
{
    public static function onConsulta($param)
    {
        try {
            ...
            // --- CORREÃ‡ÃƒO FINAL: URL ATUALIZADA PARA O NOVO ENDEREÃ‡O DO SERVIÃ‡O ---
            ...
            $criteria->add(new TFilter('placa', '=', $dados['placa']));
            $repo = new TRepository('AnttConsulta');
            $existentes = $repo->load($criteria);
            $log = ($existentes && count($existentes) > 0) ? $existentes[0] : new AnttConsulta;
            $dados['data_consulta'] = date('Y-m-d H:i:s');
            $log->fromArray($dados);
            $log->store();
            TTransaction::close();

            return ['success' => true, 'dados' => $dados, 'id' => $log->id];
