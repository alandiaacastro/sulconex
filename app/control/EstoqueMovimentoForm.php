...
class EstoqueMovimentoForm extends TPage
{
    ...
    private function buildXmlSection(): void
    {
        ...
        $head->add('<i class="fas fa-file-code fa-fw"></i> <strong>ðŸ“‚ Importar XML da NF-e</strong>
                    <span class="ms-2 badge bg-white text-info small">Preenchimento automÃ¡tico</span>');
        $sep->add($head);
        ...
        $alert = '<div class="alert alert-info py-2 small mt-1 mb-0">
            ...
            ApÃ³s importar: <strong>DANFE, Fornecedor, Importador, Peso e Itens</strong> sÃ£o preenchidos automaticamente.
        </div>';
    ...
    private function buildMovimentoSection(): void
    {
        ...
        $tipo->addItems([
            ...
            'saida'   => '<span class="text-warning fw-bold"><i class="fas fa-arrow-up"></i> SAÃDA</span>',
        ]);
        ...
        $this->form->addFields(
            ...
            [new TLabel('<i class="fas fa-calendar-alt me-1"></i> Data EmissÃ£o NF'), [$data_emi]],
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // SEÃ‡ÃƒO 3 â€“ MANIFESTO (CRT + Importador + Exportador + DANFE)
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
private function buildItensGrid(): void
    {
        ...
        $cols = [
            new TDataGridColumn('numero_item',    'NÂº',          'center', '4%'),
            new TDataGridColumn('codigo',         'CÃ³digo',       'left',   '8%'),
            new TDataGridColumn('descricao',      'DescriÃ§Ã£o',    'left',   '32%'),
    ...
    private function buildItensPanel(): TElement
    {
        ...
        $header->add('<i class="fas fa-list-ul fa-fw"></i> <strong>Itens da Nota Fiscal</strong>
                      <span class="ms-auto badge bg-white text-secondary">extraÃ­dos do XML</span>');
    ...
    private function buildObsSection(): void
    {
        ...
        $obs->setSize('100%', 70);
        $obs->setProperty('placeholder', 'ObservaÃ§Ãµes...');
        ...
        $this->form->addFields(
            ...
            [new TLabel('<i class="fas fa-comment me-1"></i> ObservaÃ§Ãµes')],                           [$obs]
        );
    }
    ...
    public function onImportarXml($param)
    {
        try {
            ...
            // Salva na sessÃ£o para usar no onSalvar
            ...
            new TMessage('info',
                "âœ… <strong>XML importado!</strong><br>"
              . "<table class='table table-sm mt-2 mb-0'>"
              . "<tr><td>ðŸ“„ DANFE</td><td><strong>{$nfe->cabecalho->danfe}</strong></td></tr>"
              . "<tr><td>ðŸ­ Fornecedor</td><td>{$nfe->emitente->nome}</td></tr>"
              . "<tr><td>ðŸ“¦ Itens</td><td><strong>{$qtd}</strong></td></tr>"
              . "<tr><td>ðŸ’° Valor Total</td><td>R$ {$vt}</td></tr>"
              . "</table>"
            );
    ...
    public function onSalvar($param)
    {
        try {
            ...
            // â”€â”€ 1. Manifesto (via CRT da tabela Conhecimento) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            // â”€â”€ 2. DANFE(s) no manifesto â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            if (!empty($data->danfe)) {
            ...
            // â”€â”€ 3. Movimento â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            // â”€â”€ 4. Itens NF-e â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            $itensNfe = TSession::getValue('estoque_nfe_itens');
            if ($itensNfe && is_array($itensNfe)) {
                // Remove itens antigos se ediÃ§Ã£o
                if (!empty($data->id)) {
                    $crit = new TCriteria();
