...
class AliquotasImpostosForm extends TPage
{
    ...
    public function __construct($param = [])
    {
        ...
        $isf        = new TNumeric('aliq_ISF',         4, ',', '.', true);

        foreach ([$irrf, $sest_senat, $pis, $cofins, $isf] as $f) {
            $f->setSize('100%');
        }
        ...
        $this->form->addFields(
            [new TLabel('Imposto s/ Faturamento (ex: 0,0282 = 2,82%)', '#FF0000')], [$isf]
        );
    ...
    public function onSave($param)
    {
        try {
            ...
            $codigos = ['IRRF', 'SEST_SENAT', 'PIS', 'COFINS', 'ISF'];
