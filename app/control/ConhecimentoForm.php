...
class ConhecimentoForm extends TPage
{
    ...
    public function __construct($param)
    {
        ...
        $tipo_cobranca->addItems([
            'FRETE_FIXO' => 'Frete Fixo',
            'POR_TONELADA' => 'Por Tonelada',
        ]);
        $valor_por_ton                           = new TNumeric('valor_por_ton', 2, ',', '.', null, true);
      ...
      $nome_destinatario     = new THidden('nome_destinatario');
        ...
        $panel_cobranca = new TPanelGroup('Cobranca');
        $panel_cobranca->style = 'margin-top: 20px';
        $this->form->addContent([$panel_cobranca]);

        $table_cobranca = new TTable;
        $table_cobranca->width = '100%';

        $row = $table_cobranca->addRow();
        $row->addCell(new TLabel('Tipo de Cobranca'))->style = 'width: 50%; padding-right: 10px;';
        $row->addCell(new TLabel('Valor por Tonelada'))->style = 'width: 50%; padding-left: 10px;';

        $row = $table_cobranca->addRow();
        $row->addCell($tipo_cobranca)->style = 'width: 50%; padding-right: 10px;';
        $row->addCell($valor_por_ton)->style = 'width: 50%; padding-left: 10px;';

        $info = new TElement('div');
        $info->style = 'margin-top:10px;color:#6c757d;font-size:12px;';
        $info->add('A tonelagem do CRT e calculada automaticamente a partir do campo Peso Bruto (kg).');

        $panel_cobranca->add($table_cobranca);
        $panel_cobranca->add($info);
        ...
        $this->form->setFields([
            $id, $permisso, $numero, $data_transportador_assinatura, $fatura_crt, $tipo_cobranca, $valor_por_ton,
    ...
    public function onSave($param)
    {
        try {
            ...
            $data->toneladas_carga = self::calculateToneladasFromPesoBruto($data->peso_bruto_kg ?? null);
            ...
            if (empty($object->numero)) {
                $object->numero = str_pad((string) $object->id, 3, '0', STR_PAD_LEFT);
                $object->store();
            }

            $data->id = $object->id;
            $data->numero = $object->numero;
    ...
    /**
     * Gerar Carta de Correcao com a ultima alteracao do CRT
     */
    public function onCartaCorrecao($param)
    {
        try {
            $id = $param['id'] ?? null;
            if (empty($id)) {
                new TMessage('warning', 'Salve o registro antes de gerar a Carta de Correcao.');
                return;
            }
            CartaCorrecaoPDF::gerar((int) $id);
    ...
    /**
     * Exibir historico de alteracoes do registro
     */
    public function onShowHistory($param)
    {
        $id = $param['id'] ?? null;
        if ($id) {
            TScript::create("__adianti_load_page('index.php?class=ConhecimentoHistoricoView&pkvalue={$id}');");
        } else {
            new TMessage('warning', 'Salve o registro antes de visualizar o historico de alteracoes.');
        }
    }
    ...
    public function onEdit($param)
    {
        try {
            if (isset($param['key'])) {
                ...
                if (empty($object->tipo_cobranca)) {
                    $object->tipo_cobranca = 'FRETE_FIXO';
                }
    ...
    private static function calculateToneladasFromPesoBruto($pesoBruto): string
    {
        return number_format(Conhecimento::convertKgToToneladas($pesoBruto), 3, '.', '');
    }
