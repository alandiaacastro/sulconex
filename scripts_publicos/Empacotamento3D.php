 ...
 * usando algoritmo height‐map otimizado.
 * Inclui opções para:
...
class Empacotamento3D
{
    ...
    public function addItem(string $tipo, int $q, float $w, float $d, float $h, float $peso_bruto)
    {
        ...
        $this->addLog("INFO", "Adicionado {$q}× {$tipo} (Base: {$w}×{$d}×{$h}, Peso: {$peso_bruto}kg). Inst. IDs: " . (self::$itemCounter - $q + 1) . " a " . self::$itemCounter);
    }
    ...
    public function generatePacking()
    {
        ...
        $this->addLog("INFO", "Iniciando carregamento... Empilhar: " . ($this->allow_stacking ? "ON" : "OFF") . 
        ...
        if (empty($itemsToPack)) { 
            $this->addLog("WARNING", "Nenhum item para carregar."); 
            return; 
        }
        ...
        if ($this->sortByVolumeDesc) { 
            ...
            $this->addLog("INFO", "Itens (instâncias) ordenados por volume original decrescente.");
        } else {
            $this->addLog("INFO", "Carregamento na ordem de entrada das instâncias (carregamento ordenado).");
        }

        $totalItemsToPack = count($itemsToPack);
        $this->addLog("INFO", "Total de instâncias para Carregar: {$totalItemsToPack}.");
        ...
        foreach ($itemsToPack as $item_idx => $itemOriginalInstance) {
            ...
            $dimsOrigStr = "{$itemOriginalInstance['w_orig']}×{$itemOriginalInstance['d_orig']}×{$itemOriginalInstance['h_orig']}";
            ...
            if ($bestPlacementOverallForThisInstance !== null) {
                ...
                $logMsg = "{$itemIdentifier} POSICIONADO como {$placedItem['tipo']} com dims {$placedItem['w']}×{$placedItem['d']}×{$placedItem['h']} em ({$placedItem['x']},{$placedItem['y']},{$placedItem['z']}).";
        ...
        $this->addLog("INFO", "Empacotamento (geral) concluído: {$packedCount}/{$totalItemsToPack} instâncias posicionadas.");
